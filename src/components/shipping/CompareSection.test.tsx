...
describe("CompareSection", () => {
  it("renders the comparison grid once the compare data resolves", async () => {
    ...
    expect(screen.getByText((content, element) => content.includes("Budget pick"))).toBeTruthy(); // per-option insight role
    expect(screen.getByText((content, element) => content.includes("Compare Service Options"))).toBeTruthy();    // anchor dimension row
