...
function inferPatchFromMessage(message: string): Partial<Record<ScalarField, unknown>> {
  ...
  const dropOffMatch = text.match(
    /\b(?:drop off|pickup|pick up|send|ship)\s+(?:on\s+)?(today|tomorrow|20\d{2}-\d{2}-\d{2}|\d{1,2}\/\d{1,2}(?:\/\d{2,4})?|(?:next\s+)?(?:monday|tuesday|wednesday|thursday|friday|saturday|sunday))\b/i,
  );
  ...
  if (
    /\b(electronics|laptop|phone|clothes|clothing|documents|books|fragile|luggage|suitcase|box|boxes|golf|skis|snowboard)\b/i.test(
      text,
    )
  ) {
...
function applyPackageToState(
  ...
  if (item.type) slots.category = item.type;
  if (item.weight) slots.weight_lbs = Number(item.weight);
  if (item.l) slots.length_in = Number(item.l);
  if (item.w) slots.width_in = Number(item.w);
  if (item.h) slots.height_in = Number(item.h);
  if (item.qty) slots.quantity = Number(item.qty);
  if (item.handling) slots.handling = item.handling;
